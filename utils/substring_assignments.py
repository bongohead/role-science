...
def flag_message_types_exact(sample_level_df: pd.DataFrame, base_messages: list[str],) -> pd.DataFrame:
    """
    Take a token-level df with columns for prompt_ix, token; identify whether each token is one of base_messages.
    ...
    Description:
        ...
        Notes:
            Tokens are concatenated *without* separators, i.e. "ab" + "cd" -> "abcd". A base_message like "bc" could match 
            across token boundaries. If you want to avoid that, ensure tokens include appropriate leading/trailing spaces 
            or other delimiters.
